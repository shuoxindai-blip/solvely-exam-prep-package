...
// Practice Test 1 is imported from the official ACT Practice Test 3 PDF by
// scripts/import-act-practice-test-3.py. Keep this generator scoped to the
// internal question-bank exam used as the diagnostic source.
await writeFile(resolve(outputRoot, 'epExams/2.json'), JSON.stringify(exams[1]))
await writeFile(resolve(outputRoot, 'epExams/index.json'), JSON.stringify({ collection: 'epExams', documents: [
  { _id: 340001, epId: EP_ID, outlineId: OUTLINE_ID, packageId: PACKAGE_ID, totalCount: 172, path: '/data/act/ep-v2/epExams/1.json' },
  { _id: exams[1]._id, epId: exams[1].epId, outlineId: exams[1].outlineId, packageId: exams[1].packageId, totalCount: exams[1].totalCount, path: '/data/act/ep-v2/epExams/2.json' },
] }))
...
console.log(JSON.stringify({ topics: topics.length, flashcards: totals.flashcards, practiceQuestions: practiceRows.length, mockExamQuestions: 172, diagnosticSourceQuestions: exams[1].totalCount, sections: Object.fromEntries(['English', 'Mathematics', 'Reading', 'Science', 'Writing'].map((section) => [section, topics.filter((topic) => topic.section === section).length])), priorities: Object.fromEntries(['CORE', 'LIKELY', 'POSSIBLE'].map((priority) => [priority, topics.filter((topic) => topic.priority === priority).length])) }, null, 2))
